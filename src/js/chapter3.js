...
List.prototype = {
    ...
    next: function () {
        // 잘못된 것으로 보인다. next() 가 listSize 까지 되지 않으면 for 문을 돌릴 수가 없다.
...
List.prototype.appendOnlyGreaterThan = function (element) {
    var isGreatThan = this.dataStore.every(function (item) {
...
List.prototype.appendOnlyLessThan = function (element) {
    var isGreatThan = this.dataStore.every(function (item) {
        if (typeof element === 'number') {
            return item > element;
        } else if (typeof element === 'string') {
...
// 3. 사람의 이름과 성별을 저장하는 Person 클래스를 구현하시오 최소한 10개의 Person 객체를 포함하는 리스트를 만드시오.
